...
q4_grades <- read.csv('grades_q4_06302016.csv', skip = 1, na.strings="")
...
q4Reading <- select(q4_gradesUnique, 7,8, 20:39)
...
q4free_read <- select(q4_gradesUnique, 7,8, 38:39)

names(q4free_read)

table(q4free_read$readFree1_quarter)

table(q4free_read$readFree2_semester)
...
for(i in mq4_read_1$readingGrade){
...
mq4_read_1 <-cbind(mq4_read_1, reading_pts2)
...
q4LangArts <- select(q4_gradesUnique, 7,8, 42:80 )
...
for(i in mq4_lang_all$langGrade){
...
mq4_lang_all <- cbind(mq4_lang_all, lang_pts2)
...
q4_lang_read <- select(q4_lang_read, council, girlCode, langCourses, langGrade, lang_pts2, readingCourses, readingGrade,reading_pts2)
...
q4_academic_data <- select(q4_academic_data, council.x, girlCode, langCourses, langGrade, lang_pts2, readingCourses,readingGrade, reading_pts2,total_absences, total_suspensions, expelled)
