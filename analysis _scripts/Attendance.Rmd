...
abs_match_success <- function() {
    abs_data_m_none <- filter(abs_match, sum_abs == 0, total_abs_final ==0)
    abs_data_m <-filter(abs_match, abs_match_trend <0)
    abs_data_2m <- nrow(abs_data_m_none) + nrow(abs_data_m)
    abs_data_1m <- abs_data_2m/nrow(abs_match)
    abs_data_percent1 <- round(abs_data_1m*100,2)
    pander(paste0(abs_data_percent1, "%", sep=""))
}
...
The number of students that could be matched is **`r nrow(abs_match)`**, and the mean attendance change from Quarter 4 to Quarter 2 is **`r mean(abs_match$abs_match_trend)`**.  The vast majority of students are improving their attendance rates. The mean difference is reflecting three studnes with a high number of abssences. The percent of students who have improved their attendance from Quarter 4 last year is **`r abs_match_success()`**


### Plot of Absences Trend

```{r echo=FALSE}

colnames(abs_match)[1] <- "Council"
colnames(abs_match)[11]<- "Absences_Change"

absence_change_plot_m <- ggplot(abs_match,(aes(x = Absences_Change, y = Council)))

absence_change_plot_m + geom_count()
