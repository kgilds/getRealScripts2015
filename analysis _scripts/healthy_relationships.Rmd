...
```{r}
pre_hr_histq <- qplot(hr_2016$Pre, geom="histogram", main= "Pre Survey", bins=40, xlab="Outcome Scores", bins=40) + geom_vline(xintercept = 4.45, col="red") 


post_hr_histq <-qplot(hr_2016$Post, geom="histogram", main= "Post Survey", bins=40, xlab="Outcome Scores", ylim=c(0,40)) + geom_vline(xintercept = 4.45, col="red")
...
```{r}
hr_bar_pre <-qplot(hr_2016$Pre >=4.45, geom="bar", ylim=c(0, 200), main="Pre survey Results", xlab="Achievment at High Level")

hr_bar_post <-qplot(hr_2016$Post >=4.45, geom ="bar", main="Post Survey Results", xlab="Achievment at High Level")
...
names(pre_parent_hr1)

pre_parent_hr1$positiveRelationships <- factor(pre_parent_hr1$positiveRelationships, levels=c("Strongly Disagree", "Disagree", "Slightly Disagree", "Neither Agree/Disagree", "Slightly Agree", "Agree", "Strongly Agree"),ordered = TRUE)

head(pre_parent_hr1)

pre_parent_hr1 <<- likert(pre_parent_hr1)

summary(pre_parent_hr1)
...
colnames(post_parent) [18] <-"relationsclassmates"
...
post_parent_hr1$positiveRelationships <- factor(post_parent_hr1$positiveRelationships, levels=c("Strongly Disagree", "Disagree", "Slightly Disagree", "Neither Agree/Disagree", "Slightly Agree", "Agree", "Strongly Agree"),ordered = TRUE)

post_parent_hr1$relationsclassmates <- factor(post_parent_hr1$relationsclassmates, levels=c("Strongly Disagree", "Disagree", "Slightly Disagree", "Neither Agree/Disagree", "Slightly Agree", "Agree", "Strongly Agree"),ordered = TRUE)

head(post_parent_hr1)

post_parent_hr1 <<- likert(post_parent_hr1)

summary(post_parent_hr1)

plot(post_parent_hr1, type="heat")
...
colnames(post_teacher) [19] <-"build_relations"
colnames(post_teacher) [16] <- "positive_relations"

post_teacher_tbl <- post_teacher %>%
    select(16, 19)

names(post_teacher_tbl)

post_teacher_tbl$build_relations <- factor(post_teacher_tbl$build_relations, levels=c("Strongly Disagree", "Disagree", "Slightly Disagree", "Neither Agree/Disagree", "Slightly Agree", "Agree", "Strongly Agree"),ordered = TRUE)

post_teacher_tbl$positive_relations <- factor(post_teacher$positive_relations, levels=c("Strongly Disagree", "Disagree", "Slightly Disagree", "Neither Agree/Disagree", "Slightly Agree", "Agree", "Strongly Agree"),ordered = TRUE)
...
plot(post_teacher_tbl, type="heat")
```


# Girl Survey Attributional

```{r}

attr_hr <- dbReadTable(getReal_2016db, "girl_post")

colnames(attr_hr)[48] <- "student_relations"
...
attr_hr$student_relations <- factor(attr_hr$student_relations, levels=c("Strongly Disagree", "Disagree", "Slightly Disagree", "Neither Agree/Disagree", "Slightly Agree", "Agree", "Strongly Agree"),ordered = TRUE)

attr_hr <- likert(attr_hr)

summary(attr_hr)

plot(attr_hr, type="heat") + ggtitle("The Get Real Program Has")
