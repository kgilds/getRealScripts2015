...
```{r filter_council, echo=FALSE, eval=TRUE}
...
The changes in grades from baseline to final grading period is represented below. The vertical axis represents baseline grades and the bottom axis the change from the baseline. Thus ideally we want to see circles moving to right. Note that,students with an A in the baseline grading period  can decrease but not increase. 
...
pander(t.test(langarts2016$baseline, langarts2016$final, paired=TRUE, alternative="less"))
