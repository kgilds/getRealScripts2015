...
```{r council_filter_reading, echo=FALSE}
...
The number of  students records that could be matched is **`r nrow(reading_2016)`** and the percent of students with a successful outcome is **`r reading_outcome()`**.
...
The changes in grades from basline to final is represented below. The vertical axis represents the baseline grading period and the bottom axis the change from Q1. Thus ideally we want to see circles moving to right. Note,students with an A in the baseline grading period can decrease but not increase. 
